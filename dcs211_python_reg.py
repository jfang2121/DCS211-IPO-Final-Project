...
panel_df = df_clean.groupby(['Country/Region Full Name', 'Month-Year']).agg({
    ...
    'first month pop': lambda x: (x.sum() / x.count()) * 100 
}).reset_index()
...
panel_df.rename(columns = {'first month pop':'first month pop%'}, inplace = True)
print(panel_df)
...
for column in ['first day pop%', 'first month pop%','ir_lt', 'ir_st', 'ue', 'cpi']:
    merged_df[f'{column}_diff'] = merged_df.groupby('Country/Region Full Name')[column].diff()

pd.set_option('display.max_columns', None)
print(merged_df)

df_filtered = merged_df.dropna(subset=['first day pop%_diff','first month pop%', 'ir_lt_diff', 'ir_st_diff', 'ue_diff', 'cpi_diff'])


merged_df['Month-Year'] = merged_df['Month-Year'].dt.to_timestamp()
merged_df = merged_df.set_index(['Country/Region Full Name', 'Month-Year'])

df_filtered = merged_df.dropna(subset=['first day pop%_diff', 'first month pop%','ir_lt_diff', 'ir_st_diff', 'ue_diff', 'cpi_diff'])

# Panel OLS for first day pop
...
results = model.fit()
print(f"first day pop% regression result:{results}")

# Panel OLS for first month pop
y2 = df_filtered['first month pop%_diff']
X2 = df_filtered[['ir_lt_diff', 'ir_st_diff', 'ue_diff', 'cpi_diff']]
X2 = sm.add_constant(X2)

model2 = PanelOLS(y2, X2, entity_effects=True,time_effects=True)
results2 = model2.fit()

print(f"first month pop% regression result:{results2}")