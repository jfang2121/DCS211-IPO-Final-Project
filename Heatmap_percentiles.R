...
# Calculate percentiles for the TotalOfferSize
ipo_aggregated$Percentile <- ecdf(ipo_aggregated$TotalOfferSize)(ipo_aggregated$TotalOfferSize)

# Create the heatmap with percentiles
ggplot(ipo_aggregated, aes(x = `Announced Date`, y = `Country/Region ISO Code`, fill = Percentile)) +
  geom_tile() +
  labs(title = "Heatmap of IPO Offer Size Percentiles", x = "Announced Date", y = "Country/Region ISO Code") +
